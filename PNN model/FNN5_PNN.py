...
# data2=np.load('Cpf1Offtarget_723_format.npz') # EN=20, BS=20
inputRow = 7
inputCol = 23
data2.files
X=data2['d1']
...
def Model():    
    ...
    x = Dropout(0.3)(x)
    x = Dense(75, activation='relu', kernel_initializer="uniform")(x)
    x = BatchNormalization()(x)
    x = Dense(50, activation='relu', kernel_initializer="uniform")(x)
    x = Dense(25, activation='relu', kernel_initializer="uniform")(x)
    x = Dropout(0.3)(x)
...
plt.title('BNN confidence plot based on FNN 5 layers')
...
plt.title('BNN forcasets with ' + '%s'%designed_conf_level +' confidence level (based on FNN 5 layers)')